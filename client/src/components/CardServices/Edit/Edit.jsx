...
import getDate from "../../FormService/getDate";
import {
  getServices,
  getWorkers,
  postOrder,
...
const Edit = () => {
  ...
  const [fieldsOnView, setFieldsOnView] = useState(-1);
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    if (selectedServices.length || selectedWorkers.length) {
      setError(
        validation(selectedServices, fieldsOnView, selectedWorkers, true)
      );
      if (!validation(selectedServices, fieldsOnView, selectedWorkers, true)) {
        const cleanArray = selectedServices.filter((service) => !!service);
        dispatch(
          postOrder({
            idVehicle: licensePlate,
    ...
    <div className={style.modalBackground}>
      <div className={style.modalContent}>
          ...
          <div className={style.dataContainer2}>
            <div className={style.servicesWorkerDiv}>
              {services.map((service) => (
                <div className={style.cardsDiv} id={service.orderService}>
        ...
        <form className={style.form} onSubmit={submitHandler}>
          ...
          <div className={style.inputDivDynamic} id="0">
            <Select
